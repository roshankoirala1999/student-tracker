...
interface Props {
  ...
  onDownloadCurrentTemplate: () => void;
}

export const MarksCsvUploadModal: React.FC<Props> = ({
  isOpen,
  sectionId,
  sectionName,
  className,
  ...
  const handleUpload = async () => {
    if (!csvContent || csvContent.trim().length === 0) {
      setError('Please select a CSV marks file to upload.');
      return;
    }
    ...
    const res = await apiRequest(`/api/sections/${sectionId}/marks/csv-import`, {
      method: 'POST',
    ...
    setLoading(false);
    if (res.success) {
      setSuccessResult(res.message || 'Marks successfully imported.');
      await onSuccess();
    } else {
      ...
      <div className="bg-white dark:bg-[#1E293B] rounded-t-3xl sm:rounded-2xl shadow-xl max-w-lg w-full border border-slate-200/80 dark:border-slate-700 max-h-[92vh] flex flex-col overflow-hidden transition-colors">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-[#F4F6FA] dark:bg-[#0F172A]">
          <div className="flex items-center gap-2 font-bold text-slate-900 dark:text-slate-100 text-sm">
            ...
            <span>Upload Marks (CSV) — {className} ({sectionName})</span>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          {/* Rules info banner */}
          <div className="bg-blue-50/80 dark:bg-blue-950/40 border border-blue-200/80 dark:border-blue-900/60 rounded-xl p-3.5 text-xs text-blue-950 dark:text-blue-200 flex items-start gap-2.5">
            <AlertTriangle className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <p className="font-semibold">Strict Marks Upload Verification</p>
              <ul className="text-[11px] text-blue-900 dark:text-blue-300/90 leading-relaxed list-disc list-inside space-y-0.5">
                <li>Download the current table template first.</li>
                <li>Student count, Roll Number, Name, and Symbol Number must match exactly.</li>
                <li>Do not add or delete students here (only enter/update scores).</li>
                <li>Marks cannot exceed each assessment&apos;s maximum mark limit.</li>
              </ul>
            </div>
          </div>

          {/* Quick download template action button */}
          <div className="flex justify-end">
            ...
            >
              ...
              <span>Download Current Marks Template (CSV)</span>
            </button>
          </div>
          ...
          >
            <input
              type="file"
              id="csv-marks-file-input"
              accept=".csv,text/csv"
              onChange={handleFileChange}
              className="hidden"
            />
            <label
              htmlFor="csv-marks-file-input"
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 dark:border-slate-700 bg-[#F4F6FA] dark:bg-[#0F172A]">
          ...
          {!successResult && (
            ...
            >
              ...
              <span>{loading ? 'Validating & Uploading...' : 'Upload & Save Marks'}</span>
            </button>
          )}
        </div>
      </div>
